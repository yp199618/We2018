          ...
          <div class="row placeholders">
          	<%-- 	<!-- 检索 -->
            	...
            	<table class="table table-bordered table-hover">
          			...
          			<c:forEach items="${page2.list}" var="order">
          				<tr>
	            			...
	            			<td>
								...
								<c:if test="${order.order_time_send=='' || order.order_time_send==null }">
									<a class="glyphicon glyphicon-ok " aria-hidden="true" title="发货" href="${pageContext.request.contextPath}/order/send?order_id=${order.order_id}"></a>
								</c:if>
								<c:if test="${order.order_time_send!='' && order.order_time_send!=null }">
									已经发货
								</c:if>
          		...
          		<div class="fenye">
          		<ul class="pagination">
					<li><a href="${pageContext.request.contextPath}/order/list?page2=1">&laquo;</a></li>
						
					<c:forEach var="i" begin="1" end="${page2.pages}">
					   <li <c:if test="${page2.pageNum==i}">class='active'</c:if>>
					   		<a href="${pageContext.request.contextPath}/order/list?page2=${i}">${i}</a>
					   	</li>
					</c:forEach>
					<li><a href="${pageContext.request.contextPath}/order/list?page2=${page2.pages}">&raquo;</a></li>
				</ul>
				</div>
				<!-- 分页结束 -->
          </div>          
